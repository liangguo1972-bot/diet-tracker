  ...
  return (
    <main className="phone page meal-page">
      <div className="page-content with-action">
        ...
        <section className="draft-section"><p className="small-label">这餐吃了什么</p><div className="draft-list">{draft.items.length === 0 && <p className="muted empty-line">还没有添加内容。</p>}{draft.items.map((item, index) => {
        ...
        })}<button className="add-row" onClick={() => onPick('cook_session')}><span aria-hidden="true">＋</span> 添加成品</button><button className="add-row" onClick={() => onPick('ingredient')}><span aria-hidden="true">＋</span> 添加单品</button></div></section>
